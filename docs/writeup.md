...
**Worked example.** `R12566` is a real KEGG reaction — the feruloyl-CoA hydrolase that converts Feruloyl-CoA → Vanillin. The agent emitted `R12566` at step 1 of its vanillin-from-ferulic-acid synthesis (Ferulic Acid → Feruloyl-CoA, the activation step that produces feruloyl-CoA, not the hydrolase step that consumes it). Right ID, wrong step. The chemistry of the cited reaction is unrelated to the chemistry of the claimed step — but the ID is real, and the citation passes existence verification. This is the failure pattern in microcosm.
