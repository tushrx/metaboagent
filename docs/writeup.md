...
We report two measurable failure modes in an evidence-grounded biochemistry agent built on Gemma 4. The first is a known capability ceiling: general-purpose vision LLMs reach 5% strict accuracy on a 20-molecule structure-recognition benchmark. The second, less-studied: existence-only citation verification — checking whether a cited reaction ID is real — hides approximately 93% real-but-wrong reaction IDs that a substrate-relevance check catches. We provide the agent (Gemma 4 family, 54k-document PubMed/KEGG/UniProt corpus, 15-tool registry, Apache 2.0), the eval harness, and a reproducible methodology. To our knowledge the second gap has not been measured systematically in prior agent benchmarks.

## Motivation

Antimalarial production is the canonical metabolic-engineering case study: the semisynthetic artemisinin route through engineered amorphadiene biosynthesis remains a textbook example of how heterologous pathway engineering supplies medicines that are otherwise plant-extraction-bound. The public-health framing has a regional dimension as well — the WHO World Malaria Report 2024 records India as accounting for 66% of malaria cases in the WHO South-East Asia Region (and 1.4% of global cases, after a 93% case-incidence reduction since 2000 that moved India out of the WHO High Burden to High Impact group in 2024)[^who2024]. India is also the largest single supplier of generic pharmaceuticals globally by volume, contributing roughly 20% of global generic-drug exports and supplying around half of US generic prescriptions by volume[^ibef2024]. Pathway-engineering tooling that lowers the floor for pathway design is therefore a public-health-relevant capability, not a purely industrial one.

[^who2024]: WHO World Malaria Report 2024 — global / regional case-share figures and India's exit from the High Burden to High Impact group. <https://www.who.int/teams/global-malaria-programme/reports/world-malaria-report-2024>
[^ibef2024]: IBEF / Indian Economic Survey 2024 — India's pharmaceutical industry is third by volume globally, eleventh by value; supplies ~20% of global generic exports and ~50% of US generics by volume. <https://www.ibef.org/industry/pharmaceutical-india>

Adjacent to that, bio-manufactured replacements for petrochemicals and overharvested natural products — vanillin, taxadiene, lycopene, isoprenoids more broadly — are studied with the same toolchain under different optimisation targets, and the climate framing flows from the same workflow. We treat health as the primary impact lens and climate as a closely coupled secondary one.
...
**Latency profile.** Pathway-design prompts (the heaviest case in the harness) complete end-to-end in **median 24.5 s** across both reasoning phases (n=45 prompt-runs across 3 independent 5-prompt runs of `eval_pathway_hallucination`, range 5.9–38.5 s; phase-1 planner median 9.4 s, phase-2 deep-dive median 14.5 s when triggered). Lookup-class prompts route through E4B and complete much faster; we do not benchmark them separately. The whole unified eval (4 evals, 8 runs, ~30 prompt-completions of varying weight) finishes in ~12 minutes thanks to vLLM prefix caching on the cache-hot prompt portions.
