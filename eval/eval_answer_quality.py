...
JUDGE_SYSTEM_PROMPT = (
    ...
    "rubric. For each rubric point INDEPENDENTLY, decide if the answer's "
    "text contains the specific information the rubric point asks about. "
    "Answer ONLY with a single JSON object on one line, no prose, no "
    "markdown, no code fences. Schema:\n"
    '  {"scores": [bool, bool, ...], "notes": "optional brief explanation"}\n'
    "\n"
    "Score each point on per-point LITERAL COVERAGE: did the answer "
    "explicitly mention the concept anywhere in its text? Do NOT penalize "
    "a point because the overall answer is incomplete, hedged, or asks "
    "the user a follow-up — score each point only on whether its specific "
    "concept appears. A concept mentioned in a strategy summary, a plan "
    "outline, or a preamble still counts as mentioned.\n"
    "\n"
    "Exception: the fabrication-check point (always last) MUST be scored "
    "FALSE if the answer cites any obvious placeholder string in place "
    "of a real ID, e.g. 'PMID: xxxxxxx', 'PMID: 12345', 'R0000', "
    "'EC ?.?.?.?', or anything similar that looks invented or templated. "
    "Substrate-relevance (whether a real-looking ID is the right one for "
    "the chemistry being discussed) is OUT OF SCOPE for this rubric — "
    "only score false on clear placeholders.\n"
    "\n"
    "Return exactly as many bool values in scores as there are rubric "
